GenerateMatchedDatasetHT <- function(exposed,
                                     candidate_matches,
                                     unit_of_observation,
  ...
  qs::qsave(unique(candidate_matches[, ..excl_cols_cand_real]),
            file.path(temporary_folder, "HT_excl_candidates_HT"), nthreads = data.table_threads)
  candidate_matches[, (excl_cols_cand) := NULL]
  qs::qsave(unique(exposed[, ..excl_cols_exp_real]),
            file.path(temporary_folder, "HT_excl_exposed_HT"), nthreads = data.table_threads)
  ...
  qs::qsave(hash_table_exact,
            file.path(temporary_folder, "HT_exact_HT"), nthreads = data.table_threads)
  ...
  for (i in 1:number_of_bootstrapping_samples) {
    set.seed(seeds[[i]])
    file_name <- file.path(temporary_folder, paste0("bootstrap_UoO_HT_", i))
